...
% extract voxel size 
voxel_size = [information.PixelSpacing; information.SliceThickness]';%%%%%% put '(for segmented part)------change-----
voxel_size
...
montage(monta);
set(gca,'clim',[0,100]);%gca gets current axis/clim 
...
% mriPartition=uint16(zeros(size(SegmentedBrainMRI)));%8
...
% Ds=flip(Ds,1);
% Ds=flip(Ds,2);
% Ds=permute(Ds,[3,2,1]);% rearranges the dimensions of an array in the order
% %specified by the vector dimorder. For example, permute(A,[2 1]) switches the
...
% daspect([voxel_size2 voxel_size2 voxel_size2]);
% %daspect auto;
% axis on;
% %camlight(-100,-100);
